...
The reasons for this, discussed in more detail in __removed link__ are to ensure both clarity and generality.
...
In every important sense, matrix types such as `Diagonal` are just as much a "matrix" as the dense matrices we have using (see the __removed link__ for more)
...
further in the __removed link__.
...
Later, we will interpret some of these in __removed link__.
...
Redo Exercise 1 using the `fixedpoint` function from `NLsolve` __removed link__.
